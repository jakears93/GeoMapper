...
if [ "$githubRelease" = true ] ; then
  ...
  lastVersion=$(tail -1 releasedVersions)
  git tag v"$version"
  git push -u origin develop
  echo "## Changelog" >> releaseNotes
  git log --pretty=format:"- %s" v"$version"..."$lastVersion" >> releaseNotes
  gh release create "$version" ./releases/*.zip -t "v$version" -n "$(head -n-1 releaseNotes)"
  success="$?"
  rm releaseNotes
  ...
  echo "v$version" >> releasedVersions
fi